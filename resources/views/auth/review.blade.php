...
$listausuarios = [
    ['id' => 1, 'nombre' => 'Empresa numero 1', 'estado' => 'activo', 'tipo' => 'mensaje', 'descripcion' => 'Esta es la descripcion de la primer empresa Esta es la descripcion de la primer empresaEsta es la descripcion de la primer empresaEsta es la descripcion de la primer empresaEsta es la descripcion de la primer empresaEsta es la descripcion de la primer empresaEsta es la descripcion de la primer empresaEsta es la descripcion de la primer empresaEsta es la descripcion de la primer empresaEsta es la descripcion de la primer empresa'],
    ['id' => 2, 'nombre' => 'Spa empresa numero 2', 'estado' => 'inactivo', 'tipo' => 'recordatorio', 'descripcion' => 'esta es la descripcion de la segunda empresa'],
    ['id' => 3, 'nombre' => 'Nombre de la empresa numero 3', 'estado' => 'activo', 'tipo' => 'error', 'descripcion' => 'esta es la descripcion de la tercer empresa']
];
...
<body style="width: 100vw; height: 100vh; margin: 0; padding: 0; background-color: #F2F3F6; display: flex; align-items: center; justify-content: center;">
    ...
    <div class="containerHome" style="width: 95vw; height: 90vh; display: flex; flex-direction: row; background-color: #FBFBFB; border-radius: 15px; box-shadow: 0 4px 6px rgba(0, 0, 0, 0.05);">
        ...
        <div style="width: 25%; height: 100%; margin-left: 3%; display: flex;  flex-direction: column;">
           
          <img src="/img/logo.svg " alt="logo Habilita.Pro" style="height:8%; margin-top:10%;">
          

          <div id="contenidovariable" style="width:100%; height:73%;">
                <div id="modulo1" style="display:none; height:100%; justify-content:space-between; padding:0;flex-direction:column;">
                    <h2 id="empresanombre" style="width:100%; margin:0; padding:0; color:#47A1A8; font-size:1.2vw; font-weight:800; "></h2>
                    ...
                    <div style="height:auto; display: grid; grid-template-columns: repeat(5, 1fr); grid-gap: 3%; padding:5%; overflow:auto;">
                    ...
                    <p id="descripcionempresa" style="height:20%;margin-top:5%; overflow:auto; font-size:.6vw; width:100%;"></p>
                    <button id="" style="font-size:1vw; width:98%; height:6.5%; color:#ffffff; border:none; border-radius:8px; background-color:#47a1a8;">Usuarios</button>
         
                </div>
          </div>
          <div id="datoscliente" style="width:100%; height:5%; display:flex; flex-direction:row; margin-top:2%;">
                <button id="configuracionesglobales" style="font-size:1vw; width:98%; height:100%; color:#ffffff; border:none; border-radius:8px; background-color:#47a1a8;">Seleccionar usuario</button>
          </div>
        ...
        <div id="slideDiv" style="padding:2%; ">
            <div style="width:100%; height:100%;overflow-y: auto; overflow-x: hidden; padding:3%; padding-top:0; padding-bottom:0;">
                <p style="margin:0; padding:0; color:#47A1A8; font-size:1.3vw; font-weight:700;">Lista de usuarios</p>
                <p style="font-size:.7vw;  padding:0;">En la siguiente lista puede encontrar todos los usuarios asignados a su cuenta como revisor de la docuentacion, puede comentar, aprobar, desaprobar la documentacion subida por el usuario.</p>

                <div style="margin-bottom: 16px; position: relative;">
                    <input type="text" id="searchInput" oninput="filtrarUsuarios()" placeholder="Buscar" style="width: 100%; padding: 6px 40px; border: 3px solid #F6F8FB; border-radius: 8px; font-size: 14px; background-color: #FAFBFE; color: #333;">
                ...
                <div id="usuarioList">
                    <?php foreach ($listausuarios as $listausuario): ?>
                        <div class="usuario-item" onclick="seleccionarusuario(<?php echo $listausuario['id']; ?>, '<?php echo $listausuario['nombre']; ?>', '<?php echo $listausuario['descripcion']; ?>')" style="width:100%; height:60px; background-color:#ffffff; border-radius:12px; box-shadow: 0 4px 8px rgba(40, 40, 40, 0.03); cursor:pointer; display:flex; flex-direction:row; padding:5%; margin-top:5px; position:relative;">
                        <div style="height:15px; aspect-ratio: 1 / 1; background-color:#47A1A8; border-radius:100%;"></div>
                            <p style="font-size:.7vw; line-height:1.2; width: calc(90% - 15px); margin-left:5%;"><?php echo $listausuario['nombre']; ?></p>
                        </div>
                    <?php endforeach; ?>
                </div>
    ...
    <script>
        ...
        function filtrarUsuarios() {
            ...
            const items = usuarioList.getElementsByClassName('usuario-item');

            for (let i = 0; i < items.length; i++) {
                const nombre = items[i].getElementsByTagName('p')[0].textContent.toLowerCase();
                items[i].style.display = (nombre.includes(filter)) ? "flex" : "none"; // Cambiar a "flex" para mantener el estilo
            }
        }

        function seleccionarusuario(a, b, c){
            slideDiv.classList.toggle('active'); 
            document.getElementById('modulo1').style.display = "flex";
            document.getElementById('empresanombre').textContent = b;
            document.getElementById('descripcionempresa').textContent = c;
        }
